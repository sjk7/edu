...
void CBaseRenderer::DisplayRendererState()
{
    ...
    DbgLog((LOG_TIMING, 1, TEXT("Current media sample %p"),m_pMediaSample));
    ...
    DbgLog((LOG_TIMING, 1, TEXT("Deliver time %s"), CDisp((LONGLONG)m_SignalTime).operator LPCTSTR()));
    ...
    DbgLog((LOG_TIMING, 1, TEXT("Last run time %s"),CDisp((LONGLONG)m_tStart.m_time).operator LPCTSTR()));
    ...
    DbgLog((LOG_TIMING, 1, TEXT("Clock time %s"), CDisp((LONGLONG)CurrentTime.m_time).operator LPCTSTR()));
...
STDMETHODIMP CBaseRenderer::GetState(DWORD dwMSecs, __out FILTER_STATE *State)
{
