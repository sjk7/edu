...
void CCritSec::Lock()
{
    ...
    if (currentOwner && (currentOwner != us)) {
        ...
        if (m_fTrace) {
            DbgLog((LOG_LOCKING, 2, TEXT("Thread %d about to wait for lock %p owned by %d"),
    ...
    if (0 == m_lockCount++) {
        ...
        if (m_fTrace) {
            DbgLog((LOG_LOCKING, tracelevel, TEXT("Thread %d now owns lock %p"), m_currentOwner, &m_CritSec));
        }
    }
}
...
void CCritSec::Unlock() {
    if (0 == --m_lockCount) {
        // about to be unowned
        if (m_fTrace) {
            DbgLog((LOG_LOCKING, 3, TEXT("Thread %d releasing lock %p"), m_currentOwner, &m_CritSec));
        }
    ...
    for (; ; ) {
        ...
        if (dwResult == WAIT_OBJECT_0 + nCount ||
            ...
            if (dwWait != INFINITE && dwWait != 0) {
                ...
                ULONGLONG dwDiff = dwNow - dwStart;
                if (dwDiff > dwWait) {
                    dwWait = 0;
                } else {
                    dwWait -= (DWORD)dwDiff;
